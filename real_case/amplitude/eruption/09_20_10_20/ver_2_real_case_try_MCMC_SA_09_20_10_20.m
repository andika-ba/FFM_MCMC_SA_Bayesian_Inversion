...
% The raw data (counts) is saved as mat file (YA.UV05.00.HHZ.2010.002.mseed.sac.mat). This raw data
% is from station UV05 and needs to be loaded. Processing steps include loading the data, converting
% the raw data (counts) to velocity, filtering the data from 5 to 15 Hz, calculating the envelope of
% the filtered data, downsampling the data (from 0.01 sec/data point to 10 sec/data point),
% smoothing using moving median.

% True model param and simulated data
load('YA.UV05.00.HHZ.2010.002.mseed.sac.mat'); % load the seismic waveform from station UV05
test  = crise(:,2)/(1.4966*800*10^6);          % convert counts to velocity
...
ENV    = abs(hilbert(filtfilt(c,d,filtfilt(a,b,test)))); % Calculate the envelope
...
res         = movmedian(crise_10sec,30); % Smooth the data over 5 minute (300 second)
     
idx_init    = (9*3600+(20*60))/10;    % Index at 9:20 am for 10 sec      
idx_1100    = (11*3600)/10;           % Index at 11:00 am for 10 sec

idx_final   = (10*3600+(20*60))/10;        % Final time (10:20) 
I_dat       = res(idx_init:idx_final);     % Picked data array
t           = 10.*(idx_init:1:idx_final);  % Picked time array 10 second increment
t_plot      = (t)./3600;
...
par_init = [80 38000 1.20];   % initial guess
bounds   = [1 37000 1.00;
            500 61200 3.00];  % lower-upper limit (boundaries)

Temp     = T_start;      
mcur     = par_init;          % guess/initial values
sd1      = 50;                % stepsize for parameter A (related to proposal distribution)
sd2      = 1000;              % stepsize for parameter tf (related to proposal distribution)
sd3      = 0.05;              % stepsize for parameter alpha (related to proposal distribution)
...
sd1_2         = 0.5;      % stepsize for parameter A (related to proposal distribution)
sd2_2         = 50;       % stepsize for parameter tf (related to proposal distribution)
...
toc
save('un_9_20_10_20_real_case_seismic_intensity.mat') % save results as a mat file
