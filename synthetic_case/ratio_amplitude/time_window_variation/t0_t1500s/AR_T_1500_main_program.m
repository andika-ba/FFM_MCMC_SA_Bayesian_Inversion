...
% and Wallace (1995). 
% In the case of amplitude ratio, we use 2 stations placed at d1 = 100m and d2 = 1000m. The amplitu-
% de ratio values are the ratio between amplitudes from the 2 stations. 

v    = 1;              % Magma velocity/upward vertical velocity of the migrating source (m/s)
d1   = 100;            % Distance between the 1st seismic station and the eruptive site (meters)
d2   = 1000;           % Distance between the 2nd station and the eruptive site (meters)
z0   = -2000;          % Initial depth of migrating magma/migrating seismic source (meters)
t0   = 0;              % Initial time (second)
tend = 1500;           % Time at the end of seismic amplitude recording (seconds)
...
r2   = sqrt((z.^2)+(d2^2)); % Radial distance array (meters), using Pythagorean Theorem
...
Magn = 1.0;            % Earthquake Magnitude
I0n1 = sqrt(10^((1.5*Magn)+11.8)); % Seismic amplitude of the source (Lay and Wallace, 1995)

y_in_1 = (I0n1.*exp(-pi*f.*r1./(Q*beta)))./(r1.^n); % Synthetic dataset generated from 1st station
y_in_2 = (I0n1.*exp(-pi*f.*r2./(Q*beta)))./(r2.^n); % Synthetic dataset generated from 2nd station
y_in   = y_in_1./y_in_2; % Amplitude Ratio
...
bounds   = [1E-8 1000 1.00;1E-3 4000 6.00];  % lower-upper limit (boundaries)
Temp     = T_start;      
mcur     = par_init;      % guess/initial values
sd1      = 5E-5;       % stepsize for parameter A (related to proposal distribution)
sd2      = 50;            % stepsize for parameter tf (related to proposal distribution)
sd3      = 0.1;          % stepsize for parameter alpha (related to proposal distribution)
...
sd1_2         = 1E-6;     % stepsize for parameter A (related to proposal distribution)
sd2_2         = 10;       % stepsize for parameter tf (related to proposal distribution)
sd3_2         = 0.5;      % stepsize for parameter alpha (related to proposal distribution)
...
toc
save('AR_t_1500_seismic_intensity_try_hybrid.mat') % save results as a mat file
