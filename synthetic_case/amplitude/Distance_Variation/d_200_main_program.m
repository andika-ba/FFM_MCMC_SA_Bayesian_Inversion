...
% and Wallace (1995)
% In this variation, the distance between the seismic station and the eruptive site (d1) is 200
% meters.

v    = 1;              % Magma velocity/upward vertical velocity of the migrating source (m/s)
d1   = 200;            % Distance between the seismic station and the eruptive site (meters)
...
y_in = (I0n1.*exp(-pi*f.*r1./(Q*beta)))./(r1.^n); % Synthetic dataset generated
...
bounds   = [1E-8 1000 1.00;1E-3 3000 3.00];  % lower-upper limit (boundaries)
Temp     = T_start;      
mcur     = par_init;      % guess/initial values
sd1      = 1E-6;          % stepsize for parameter A (related to proposal distribution)
sd2      = 100;           % stepsize for parameter tf (related to proposal distribution)
...
toc
save('d_1200_seismic_intensity_try_hybrid_1900.mat') % save results as a mat file
